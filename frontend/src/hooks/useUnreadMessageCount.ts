 ...
 * Hook to manage unread message count
    ...
    For every channel member, we store a last_visit timestamp in the Chatly Channel Member doctype. To get the number of unread messages, we can simply look at the no. of messages created after this timestamp for any given channel.
    ...
    3. When the user sends a new message (handled by controller under Chatly Message)
    4. When the user exits a channel after all the latest messages had been loaded (called from the frontend)

    When Chatly loads, we fetch the unread message counts for all channels. Post that, updates to these counts are made when:
...
const useUnreadMessageCount = () => {
    ...
    const { data: unread_count, mutate: updateCount } = useFrappeGetCall<{ message: UnreadCountData }>("chatly.api.chatly_message.get_unread_count_for_channels",
        undefined,
        ...
        updateCount(d => {
            if (d) {
                ...
                if (d.message.channels.find(c => c.name === channelID)) {
                    return call.get('chatly.api.chatly_message.get_unread_count_for_channel', {
    ...
    useFrappeEventListener('chatly:unread_channel_count_updated', (event) => {
        ...
        if (event.sent_by !== currentUser) {
            ...
            if (channelID === event.channel_id && !state?.baseMessage) {
                ...
                // https://github.com/The-Commit-Company/Chatly/pull/745#issuecomment-2014313429
    ...
    useEffect(() => {
        // @ts-expect-error
        const app_name = window.app_name || "Chatly"
        if (unread_count) {
