...
const AddChatlyUsersContent = ({ onClose }: { onClose: VoidFunction }) => {
    ...
    const chatlyUsersArray = users.enabledUsers.map(user => user.name)

    const [selected, setSelected] = useState<string[]>([])
    const { loading, call, error: postError } = useFrappePostCall<{ message: AddUsersResponse }>('chatly.api.chatly_users.add_users_to_chatly')
        ...
        if (selected.length > 0) {
            ...
            call({
                ...
                if (res.message.success_users.length !== 0) {
                    toast.success(`You have added ${res.message.success_users.length} users to Chatly`)
                }

                mutate('chatly.api.chatly_users.get_list')
    ...
    return (
        <div>
            <Dialog.Title>Add users to Chatly</Dialog.Title>
            ...
            <Flex direction='column' gap='4' pt='4'>
                ...
                {failedUsers.length > 0 && <ErrorCallout>
                    Could not add the following users to Chatly since they have a <Strong>Role Profile</Strong> attached.<br />
                ...
                {data && data.length !== 0 && <UsersTable data={data} defaultSelected={chatlyUsersArray} selected={selected} setSelected={setSelected} />}
...
export default AddChatlyUsersContent