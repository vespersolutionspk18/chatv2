...
import { ChatlyPoll } from "@/types/ChatlyMessaging/ChatlyPoll"
import { ErrorBanner } from "@/components/layout/AlertBanner"
import { ChatlyPollOption } from "@/types/ChatlyMessaging/ChatlyPollOption"
...
export const PollMessageBlock = ({ message, user, ...props }: PollMessageBlockProps) => {
    ...
    const { data, error, mutate } = useFrappeGetCall<{ message: Poll }>('chatly.api.chatly_poll.get_poll', {
    ...
    useFrappeDocumentEventListener('Chatly Poll', message.poll_id, () => {
        mutate()
    })
...
const PollOption = ({ data, option }: { data: Poll, option: ChatlyPollOption }) => {
...
const SingleChoicePoll = ({ data, messageID }: { data: Poll, messageID: string }) => {
    ...
    const { call } = useFrappePostCall('chatly.api.chatly_poll.add_vote')
    const onVoteSubmit = async (option: ChatlyPollOption) => {
        return call({
