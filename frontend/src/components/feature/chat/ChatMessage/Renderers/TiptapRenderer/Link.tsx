...
export const LinkPreview = memo(({ messageID }: { messageID: string }) => {
    ...
    const { data } = useFrappeGetCall<{ message: LinkPreviewDetails[] }>('chatly.api.preview_links.get_preview_link', {
    ...
    const { call } = useFrappePostCall('chatly.api.preview_links.hide_link_preview')
