...
const useChatStream = (scrollRef: MutableRefObject<HTMLDivElement | null>) => {
    ...
    const { call: fetchOlderMessages, loading: loadingOlderMessages } = useFrappePostCall('chatly.api.chat_stream.get_older_messages')
    const { call: fetchNewerMessages, loading: loadingNewerMessages } = useFrappePostCall('chatly.api.chat_stream.get_newer_messages')
    ...
    const { data, isLoading, error, mutate } = useFrappeGetCall<GetMessagesResponse>('chatly.api.chat_stream.get_messages', {
    ...
    const { call: trackVisit } = useFrappePostCall('chatly.api.chatly_channel_member.track_visit')
    /**
     ...
     *
     * Since we are using Web Socket events, this saves multiple round-trips to the server to fetch messages (pre Chatly v1.5)
     *
    ...
    useFrappeDocumentEventListener('Chatly Channel', channelID ?? '', () => { })
