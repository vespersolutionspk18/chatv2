...
export const WebhookItem = ({ webhook, mutate }: { webhook: ChatlyWebhook, mutate: () => void }) => {
                ...
                <Flex direction={'row'} gap={'2'} align={'center'}>
                    ...
                    <AlertDialog.Root open={open} onOpenChange={setOpen}>
                        ...
                        <AlertDialog.Content className={DIALOG_CONTENT_CLASS}>
                            <AlertContent doctype="Chatly Webhook" docname={webhook.name} onClose={onClose} onUpdate={mutate} />
                        </AlertDialog.Content>
                    </AlertDialog.Root>
                </Flex>
            </Flex>
...
const DeleteWebhookAlertContent = ({ webhhookID, onClose, mutate }: { webhhookID: string, onClose: () => void, mutate: () => void }) => {
    ...
    const onDelete = () => {
        deleteDoc('Chatly Webhook', webhhookID).then(() => {
            mutate()
            onClose()
