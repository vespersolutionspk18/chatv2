...
export type ChannelListItem = Pick<ChatlyChannel, 'name' | 'channel_name' | 'type' |
...
export const useFetchChannelList = (): ChannelListContextType => {
    ...
    const { data, mutate, ...rest } = useFrappeGetCall<{ message: ChannelList }>("chatly.api.chatly_channel.get_all_channels", {
        hide_archived: false
    }, `channel_list`, {
    ...
    useFrappeDocTypeEventListener('Chatly Channel', () => {
        mutate()
    ...
    const updateLastMessageInChannelList = async (channelID: string) => {
        ...
        globalMutate(`channel_list`, async (channelList?: { message: ChannelList }) => {
            if (channelList) {
                ...
                if (isChannelPresent) {
                    return call.get('chatly.api.chatly_channel.get_last_message_details', {
